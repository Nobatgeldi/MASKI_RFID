	...
	: mfrc522(NULL)
{
	...
	byte NFCKeyAByteArray[] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
	...
	byte MADKeyAByteArray[] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
...
RFID_Utilities::RFID_Utilities(MFRC522 *mfrc522)
	: mfrc522(mfrc522)
{
	byte NFCKeyADefaultByteArray[] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
	copyBytesToKey(NFCKeyADefaultByteArray, &NFCDefaultKeyA);
...
void RFID_Utilities::setMFRC(MFRC522 *mfrc522)
{
	this->mfrc522 = mfrc522;
}

void RFID_Utilities::showReaderDetails()
{
...
bool RFID_Utilities::detectNewCard()
{
	// Look for new cards
...
void RFID_Utilities::copyBytesToKey(byte *array, MFRC522::MIFARE_Key *key)
{
	for (int i = 0; i < 6; i++)
...
void RFID_Utilities::printAllSectors(MFRC522::MIFARE_Key *MADKeyA, MFRC522::MIFARE_Key *NFCKeyA)
{
	byte sector = 0;
	printSector(sector, MADKeyA, MADKeyA);

	for (sector = 1; sector <= 15; sector++) {
		printSector(sector, NFCKeyA, MADKeyA);
	}
}

void RFID_Utilities::dump_byte_array(byte *buffer, byte bufferSize)
{
...
void RFID_Utilities::writeBlock(byte sector, byte blockAddr, byte* dataBlock, MFRC522::MIFARE_Key *key)
{
	byte trailerBlock = sector * 4 + 3;
	Serial.println(F("Authenticating using key B..."));
	MFRC522::StatusCode status = mfrc522->PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_B, trailerBlock, key, &(mfrc522->uid));
	if (status != MFRC522::STATUS_OK) {
		Serial.print(F("PCD_Authenticate() failed: "));
		Serial.println(mfrc522->GetStatusCodeName(status));
		return;
...
void RFID_Utilities::formatToNDEF(MFRC522::MIFARE_Key *OldMADKeyB, MFRC522::MIFARE_Key *OldNFCKeyB)
{
	...
	// and the GPB is 0xC1. FF078069
	byte dataBlock03[] = { 0xA1, 0xA2, 0xA3, 0xA4,
	  0xA5, 0xFF, 0xFF, 0x07,
	  0x80, 0x69, 0xFF, 0xFF,
	  0xFF, 0xFF, 0xFF, 0xFF
	};
	...
	// and the GPB is 0x40.
	byte nfcTrailerBlock[] = { 0xA1, 0xA2, 0xA3, 0xA4,
	  0xA5, 0xFF, 0xFF, 0x07,
	  0x80, 0x69, 0xFF, 0xFF,
	  0xFF, 0xFF, 0xFF, 0xFF
	};
	...
	writeBlock(0, 2, dataBlock02, OldMADKeyB);
...
byte RFID_Utilities::textToNDEFMessage(char* text, byte* result)
{
...
void RFID_Utilities::writeByteToTag(byte b, byte blockAddr, MFRC522::MIFARE_Key *keyB) {
...
byte RFID_Utilities::readByteFromTag(byte blockAddr, MFRC522::MIFARE_Key *keyA) {
...
void RFID_Utilities::writeTextToTag(char* text, byte sector, byte blockAddr, MFRC522::MIFARE_Key *keyB) {
	byte* NDEFText;
...
bool RFID_Utilities::tryKeyOnSector(byte command, byte sector, MFRC522::MIFARE_Key *key)
{
	byte trailerBlock = sector * 4 + 3; // get the trailer block
...
void RFID_Utilities::tryKeyADictionaryOnSector(byte sector)
{
	int numOfKeys = 2;
...
void RFID_Utilities::writeDefaultMessage() {
	/* sector 1 block 4 */
...
void RFID_Utilities::writePlainText() {
	/* sector 1 block 4 */
	byte dataBlock14[] = { 0x03, 0x17, 0xD1, 0x01,
	  0x13, 0x54, 0x02, 0x65,
	  0x6E, 0x61, 0x62, 0x63,
